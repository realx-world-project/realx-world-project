...
export default function NewMaterialListingClient() {
  ...
  const onSubmit = async (data: MaterialListingFormData) => {
    ...
    try {
      ...
      for (let i = 0; i < doneImages.length; i++) {
        ...
        await fetch(`/api/materials/${listing.id}/images`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ url: img.url, publicId: img.publicId, isPrimary: i === 0 }),
        });
      }
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Add Material Listing</h1>
        <p className="mt-2 text-muted-foreground">List a building material for buyers to discover</p>
      </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g., Dangote Cement (50kg bag)" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea rows={5} placeholder="Describe the material, grade, and delivery options…" {...field} />
                    </FormControl>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                  {imageItems.map((item, index) => (
                    <div key={item.id} className="group relative aspect-square overflow-hidden rounded-lg border">
                      <img src={item.previewUrl} alt={`Preview ${index + 1}`} className="h-full w-full object-cover" />
              ...
              {imageItems.length > 0 && (
                <p className="text-sm text-muted-foreground">
                  ...
                  {uploadingCount > 0 && <span className="ml-2 text-blue-500">· {uploadingCount} uploading…</span>}
